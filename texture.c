...
void init()
{
    ...
    angle = 0.0;
	 makeIndices();
	 int number = makeSphere(1.2,0.0,0.0,0.0);
	 bindBuffers();
    //glUniform3f(getUniLoc(prog, "Light.Intensity"),1.0f,1.0f,1.0f );
...
void setMatrices()
{
    ...
    glUniformMatrix4fv(getUniLoc(prog, "V"),1, GL_FALSE,  &mv[0][0]);//ViewMatrix
    glUniformMatrix4fv(getUniLoc(prog, "M"),1, GL_FALSE,  &model[0][0]);//ModelViewMatrix
    glUniformMatrix3fv(getUniLoc(prog, "MV3x3"), 1, GL_FALSE, &normalMatrix[0][0]);//NormalMatrix
...
void render()
{
    ...
    /*glUniform3f(getUniLoc(prog, "Material.Kd"), 0.9f, 0.9f, 0.9f);
    glUniform3f(getUniLoc(prog, "Material.Ks"), 0.95f, 0.95f, 0.95f);
    glUniform3f(getUniLoc(prog, "Material.Ka"), 0.1f, 0.1f, 0.1f);
    glUniform1f(getUniLoc(prog, "Material.Shininess"), 100.0f);*/
