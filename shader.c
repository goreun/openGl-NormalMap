...
char * readShader(const char *fileName, ShaderType shaderType)
{
    FILE *fp;
    char name[20];
    char *vcontent = NULL;
...
int installShaders()
{
   ...
   vShader = readShader("texture.vert", EVertexShader);
   ...
   fShader = readShader("texture.frag", EFragmentShader);
  ...
  glBindAttribLocation(programHandle, 2, "VertexTangent");
  glBindAttribLocation(programHandle, 3, "VertexBitangent");
  glBindAttribLocation(programHandle, 4, "VertexTexCoord");
